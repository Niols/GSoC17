/*
 * Copyright (C) 2014, United States Government, as represented by the
 * Administrator of the National Aeronautics and Space Administration.
 * All rights reserved.
 *
 * Symbolic Pathfinder (jpf-symbc) is licensed under the Apache License,
 * Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 *        http://www.apache.org/licenses/LICENSE-2.0.
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

//
//Copyright (C) 2006 United States Government as represented by the
//Administrator of the National Aeronautics and Space Administration
//(NASA).  All Rights Reserved.
//
//This software is distributed under the NASA Open Source Agreement
//(NOSA), version 1.3.  The NOSA has been approved by the Open Source
//Initiative.  See the file NOSA-1.3-JPF at the top of the distribution
//directory tree for the complete NOSA document.
//
//THE SUBJECT SOFTWARE IS PROVIDED "AS IS" WITHOUT ANY WARRANTY OF ANY
//KIND, EITHER EXPRESSED, IMPLIED, OR STATUTORY, INCLUDING, BUT NOT
//LIMITED TO, ANY WARRANTY THAT THE SUBJECT SOFTWARE WILL CONFORM TO
//SPECIFICATIONS, ANY IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR
//A PARTICULAR PURPOSE, OR FREEDOM FROM INFRINGEMENT, ANY WARRANTY THAT
//THE SUBJECT SOFTWARE WILL BE ERROR FREE, OR ANY WARRANTY THAT
//DOCUMENTATION, IF PROVIDED, WILL CONFORM TO THE SUBJECT SOFTWARE.
//

package gov.nasa.jpf.symbc.heap.seplogic.constraint;

import java.util.Map;
import java.util.HashMap;
import java.util.StringJoiner;

public class Record extends AbstractInformation
{
    private final Map<String,Node> fields;
    
    public Record(Map<String,Node> fields) {
	this.fields = fields;
    }

    @Override
    public boolean isRecord() {
	return true;
    }
    
    public Map<String,Node> getFields() {
	return this.fields;
    }

    public void updateField(String field, Node newNode) {
	this.fields.put(field, newNode);
    }
    
    @Override
    public Information unifyWith(Information other, Node node, boolean areSeparated) throws UnsatException {

	if (other.isNil()) {
	    throw new UnsatException();
	}
	
	else if (other.isRecord()) {
	    if (areSeparated) {
		throw new UnsatException();
	    }
	    else {
		Record otherRecord = (Record) other;

		Map<String,Node> newFields = new HashMap<String,Node>(this.fields);

		for (Map.Entry<String,Node> otherEntry : otherRecord.getFields().entrySet()) {
		    String otherName = otherEntry.getKey();
		    Node otherNode = otherEntry.getValue();

		    Node thisNode = this.fields.get(otherName);
		    if (thisNode == null) {
			this.fields.put(otherName, otherNode);
		    } else {
			thisNode.union(otherNode);
		    }
		}

		return new Record(newFields);
	    }
	}

	else if (other.arePredicates()) {
	    return other.unifyWith(this, node, areSeparated);
	}

	else {
	    throw new UnsatException("This case should not happen: the system is unsound.");
	}
    }

    public String toString() {
	StringJoiner commaJoiner = new StringJoiner(", ");
	for (Map.Entry<String,Node> entry : this.fields.entrySet()) {
	    commaJoiner.add(entry.getKey() + "=" + entry.getValue().toString());
	}
	return "{|" + commaJoiner.toString() + "|}";
    }
    
    public String toString(Node node) {
	return node.toString() + "->" + this.toString();
    }
}
